                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-2 mt-1">
                                ...
                                <Badge variant="outline" className={`gap-1.5 pl-1.5 ${statusConfig.softColor === 'success' ? 'bg-green-50 text-green-700 border-green-200' : statusConfig.softColor === 'error' ? 'bg-red-50 text-red-700 border-red-200' : 'bg-amber-50 text-amber-700 border-amber-200'}`}>
                                    <span className={`h-1.5 w-1.5 rounded-full ${statusConfig.softColor === 'success' ? 'bg-green-500' : statusConfig.softColor === 'error' ? 'bg-red-500' : 'bg-amber-500'}`} />
                                    {statusConfig.label}
                                </Badge>
                            </div>
                        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <Card>
                    ...
                    <CardContent className="grid grid-cols-2 gap-y-4 text-sm">
                        <SpecItem label="Hệ điều hành" value={device.deviceInfo.os} icon={<Laptop className="h-3.5 w-3.5" />} />
                        <SpecItem label="CPU" value={device.deviceInfo.cpu} icon={<Cpu className="h-3.5 w-3.5" />} />
                        <SpecItem label="RAM" value={device.deviceInfo.ram} icon={<HardDrive className="h-3.5 w-3.5" />} />
                        <SpecItem label="Kiến trúc" value={device.deviceInfo.architecture} icon={<Monitor className="h-3.5 w-3.5" />} />
                        <SpecItem label="Địa chỉ MAC" value={device.deviceInfo.mac} icon={<Network className="h-3.5 w-3.5" />} />
                    </CardContent>
                </Card>
                        ...
                        {device.assignment ? (
                            <div className="flex items-center justify-between p-3 bg-muted/50 rounded-lg border">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p className="font-medium text-sm">{device.assignment.assignee_name}</p>
                                        <p className="text-xs text-muted-foreground">{device.assignment.assignee_email}</p>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <div className="text-[10px] text-muted-foreground uppercase mb-0.5">Ngày gán</div>
                            ...
                            <div className="flex flex-col items-center justify-center py-6 text-center bg-muted/20 rounded-lg border border-dashed">
                                <p className="text-sm text-muted-foreground mb-2">Thiết bị chưa được gán</p>
                            </div>
                        )}
...
function SpecItem({ label, value, icon }: { label: string, value?: string, icon: React.ReactNode }) {
    return (
        <div className="space-y-1">
            ...
            <dd className="text-sm font-medium pl-5">{value || "—"}</dd>
        </div>
    );
}
